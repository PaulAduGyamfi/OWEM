# OWEM Color Palette — v4 (iOS-leaning, lime accent)

iOS system neutrals and status colors stay; the accent returns to the brand's own **acid lime** — distinctiveness won over convention.

## Why lime (`#C9F31D`, both modes)

- **It's the brand.** The logo, landing page and exports already own it — one accent everywhere compounds recognition (consistent color use is tied to ~80% brand-recognition lift).
- **Fastest hue the eye resolves** — high-chroma yellow-green grabs attention without red's alarm. Spent on ONE action per screen so it never dulls.
- **Category-ownable**: Venmo blue, Cash App green, Splitwise teal — nobody owns acid lime.
- **Zero collision by discipline, not hue**: lime sits near systemGreen, so the rule is strict — lime is only ever the action tint; money-in is always systemGreen *on its soft pill*, never bare. The pill shape + darker text keeps them unmistakable.
- Same value works on white and true black — no dark-mode variant needed.

## Light mode

| Token | Hex | iOS equivalent / use |
|---|---|---|
| `bg` | `#F2F2F7` | systemGroupedBackground — app background |
| `surface` | `#FFFFFF` | secondarySystemGroupedBackground — cards, sheets |
| `surface-alt` | `#E5E5EA` | tertiarySystemFill — chips, segmented controls, tab bar |
| `ink` | `#000000` | label — primary text, primary buttons |
| `ink-secondary` | `#6D6D72` | secondaryLabel — body/secondary text |
| `ink-tertiary` | `#AEAEB2` | tertiaryLabel — timestamps, placeholders |
| `border` | `#C6C6C8` | separator — hairlines (use at 0.5px like iOS) |
| `accent` | `#C9F31D` | Brand lime — THE action: primary CTA, active tab, tint color |
| `accent-ink` | `#000000` | Text/icons on lime |
| `accent-soft` | `#F4FBD8` | Selected states, lime tint fills |
| `positive` | `#34C759` | systemGreen — fills, icons, large numerals |
| `positive-text` | `#248A3D` | Accessible green — positive text at body sizes |
| `positive-soft` | `#E9F8EE` | Positive pill/row background |
| `negative` | `#FF3B30` | systemRed — fills, icons, large numerals |
| `negative-text` | `#D70015` | Accessible red — negative text at body sizes |
| `negative-soft` | `#FFEBE9` | Negative pill/row background |
| `warning` | `#FF9500` | systemOrange — pending fills, icons |
| `warning-text` | `#B25000` | Pending text at body sizes |
| `warning-soft` | `#FFF4E5` | Pending pill background |

## Dark mode

| Token | Hex | iOS equivalent / use |
|---|---|---|
| `bg` | `#000000` | systemGroupedBackground (dark) — true black, OLED-friendly |
| `surface` | `#1C1C1E` | secondarySystemGroupedBackground — cards, sheets |
| `surface-alt` | `#2C2C2E` | tertiarySystemGroupedBackground — chips, tab bar |
| `ink` | `#FFFFFF` | label — primary text |
| `ink-secondary` | `#A1A1A6` | secondaryLabel |
| `ink-tertiary` | `#636366` | tertiaryLabel |
| `border` | `#38383A` | separator |
| `accent` | `#C9F31D` | Same lime — born for true black |
| `accent-ink` | `#000000` | Text/icons on lime |
| `accent-soft` | `#23290A` | Selected states, lime tint fills |
| `positive` | `#30D158` | systemGreen (dark) |
| `positive-soft` | `#122A1A` | Positive row background |
| `negative` | `#FF453A` | systemRed (dark) |
| `negative-soft` | `#2E1514` | Negative row background |
| `warning` | `#FF9F0A` | systemOrange (dark) |
| `warning-soft` | `#2B2110` | Pending row background |

## iOS conventions to carry with the palette

- **Tint color**: lime is the app-wide tint — active tab icons, toggles, selection — exactly how iOS apps use one tint (links use `ink` since lime text fails contrast).
- **Hairlines at 0.5px** with `border`, never 1px gray boxes.
- **Materials**: sheets and notification banners use blur (`backdrop-filter`) over `surface` at ~90% opacity, not opaque panels.
- **Elevation by background, not shadow**: dark mode layers `bg → surface → surface-alt`; shadows are for light mode only, soft and low.
- **SF Pro** (or system font stack) with Dynamic-Type-ish scale; numerals in bold weights, labels a step down.

## Usage rules

1. **One lime per screen.** Primary action only; second-tier actions use `ink` (light) / `surface-alt` (dark).
2. **On lime, always black.** Never lime text on white — it fails contrast; lime is a surface, not a text color.
3. **Status colors never decorate** — only money direction or state, always on their soft pills with the `-text` variants at body sizes. This is what keeps lime (action) and systemGreen (money in) unmistakable.
4. **Numbers get the strongest ink**; the amount is always the loudest thing on screen.
5. **The reward moment**: "paid you" pairs systemGreen on its soft tint with one lime action — the dopamine hit is associated with opening OWEM.
