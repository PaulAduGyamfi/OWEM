...
export function ReceiptCamera({
  onCapture, onManual, onClose, busy = false,
}: {
  ...
  const fromLibrary = async () => {
    ...
    if (!picked.canceled && picked.assets[0]?.uri) onCapture(picked.assets[0].uri);
  };
  ...
  if (!permission.granted) {
    return (
      <View style={{ flex: 1, backgroundColor: '#000000', paddingTop: insets.top }}>
        <Header close tint={TINT} home={false} onBack={onClose} />
        ...
        <View style={{ paddingHorizontal: space[4], paddingBottom: insets.bottom + space[6], gap: space[3] }}>
          ...
          <Button label="Type it in instead" variant="secondary" onPress={onManual} />
        </View>
      </View>
    );
  }
    ...
    <View style={{ flex: 1, backgroundColor: '#000000', paddingTop: insets.top }}>
      ...
      >
        ...
        >
          ...
          >
            {working && <ActivityIndicator color="#000000" />}
          </View>
        </Press>

        <Press onPress={onManual} disabled={working} style={{ width: 56 }}>
